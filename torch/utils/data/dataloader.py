...
class _DatasetKind(object):
    ...
    @staticmethod
    def create_fetcher(kind, worker_id, dataset, auto_collation, collate_fn, drop_last):
        if kind == _DatasetKind.Map:
            return _utils.fetch._MapDatasetFetcher(worker_id, dataset, auto_collation, collate_fn, drop_last)
        else:
            return _utils.fetch._IterableDatasetFetcher(worker_id, dataset, auto_collation, collate_fn, drop_last)
        ...
        for i in range(self._num_workers):
            ...
            w = multiprocessing_context.Process(
                ...
                args=(self._dataset_kind, self._dataset, index_queue,
                      ...
                      self._persistent_workers, self._prefetch_factor, self._super_batch))
            w.daemon = True
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        while True:
            ...
            # Could we statefully store some of the data (e.g., X-batches) and then pull from this state until its empty?
    ...
    # FUNCTION THAT ADDS DATA TO WORKER INPUT QUEUES
    def _try_put_index(self):
        ...
        self._index_queues[worker_queue_idx].put((self._send_idx, index)) # !!! HERE !!!
